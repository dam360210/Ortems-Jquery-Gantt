...
//■コア部分
        ...
        var core = {
            ...
            render: function (element) {
                ...
                content.append("<div class='fn-gantt.sn'></div>");
            ...
            leftPanel: function (element) {
                ...
                var ganttLeftPanel = $('<div class="leftPanel"/>')
                    .append(
                        $('<div class="row spacer" onclick="fnc()">'+settings.plant+'</div>')
                ...
                switch (settings.scale) {
                    ...
                    case "hours":
                        ...
                        for (i = 0, len = range.length; i < len; i++) {
                            ...
                            if (rgetDay !== getDay) {
                                ...
                                dowArr.push(
                                    ...
                                    'style="width: ' + tools.getCellSize() * hoursInDay + 'px;">' +
                                    '<div class="fn-label">' + day.getFullYear() + '/' + settings.months[day.getMonth()] + '/' + day.getDate() + settings.dow[getDay] + '</div></div>');
                        ...
                        dowArr.push(
                            '<div class="row day ' + dayClass + '" ' +
                            'style="width: ' + tools.getCellSize() * hoursInDay + 'px;">' +
                            '<div class="fn-label">' + day.getFullYear() + '/' + settings.months[day.getMonth()]+ '/' + day.getDate() +  settings.dow[day.getDay()] + '</div></div>');
            ...
            createProgressBar: function (label, desc, classNames, dataObj) {
                label = label || "";
                var mytilte = label.replace('<br>', '\r\n') ;
                var bar = $('<div class="bar"><div class="fn-label" title="' + mytilte + '">' + label + '</div></div>')
                    ...
                    if (i >= element.pageNum * settings.itemsPerPage &&
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                // **Hourly data**
                                case "hours":
                                    //棒のfrom時点の「Hr」に応じてのDIV_IDを取得
                                    dFrom = tools.genId(tools.dateDeserialize(day.from), element.scaleStep);
                                    //DIV_IDによって、DIVエレメントをGET
                                    from = $(element).find('#dh-' + dFrom);

                                    //棒のto時点の「Hr」に応じてのDIV_IDを取得
                                    dTo = tools.genId(tools.dateDeserialize(day.to), element.scaleStep);
                                    //DIV_IDによって、DIVエレメントをGET
                                    to = $(element).find('#dh-' + dTo);

                                    //fromのoffset = hrのoffset + mimutesのoffset
                                    cFrom = from.data("offset")+tools.dateDeserialize(day.from).getMinutes()*cellWidth/60;

                                    //toのoffset = hrのoffset + mimutesのoffset
                                    cTo = to.data("offset")+tools.dateDeserialize(day.to).getMinutes()*cellWidth/60;
